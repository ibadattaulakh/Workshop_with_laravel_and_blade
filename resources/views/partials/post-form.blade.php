<!-- resources/views/partials/post-form.blade.php -->
<div class="{{ $containerClass ?? 'flex items-start gap-4' }}">
  ...
  <form method="POST" action="{{ $action ?? '#' }}" class="grow" @if(empty($action) || $action === '#') onsubmit="event.preventDefault(); return false;" @endif>
    @csrf
    ...
    <textarea
      id="{{ $fieldName }}"
      name="{{ $fieldName }}"
      class="{{ $textareaClass ?? 'w-full resize-none text-lg' }}"
    ...
    <div class="flex items-center justify-between gap-4">
      ...
      <button
        type="{{ (empty($action) || $action === '#') ? 'button' : 'submit' }}"
        class="{{ $buttonClass ?? 'bg-pixl hover:bg-pixl/90 active:bg-pixl/95 text-pixl-dark border border-transparent px-4 py-1 text-sm' }}"
      >
        Post
      </button>
    </div>
