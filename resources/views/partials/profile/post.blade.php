<li class="flex items-start gap-4 not-first:pt-2.5">
  <a href="{{ route('profiles.show', $item->profile->handle) }}" class="shrink-0">
    <img
    ...
    <div class="border-pixl-light/10 border-b pb-5">
      ...
      <div class="flex items-center justify-between gap-4">
        <div class="flex items-center gap-2.5">
          <p><a class="hover:underline" href="{{ route('profiles.show', $item->profile->handle) }}">{{ $item->profile->display_name }}</a></p>
          ...
          <p>
            <a
              ...
              href="{{ route('profiles.show', $item->profile->handle) }}"
      ...
      <div class="mt-4 flex flex-col gap-3 text-sm">
        @if ($item->isRepost())
          @if (blank($item->content) && $item->repostOf)
            {{-- pure repost (no extra content) — render the reposted post compactly --}}
            @include('partials.profile.post', ['item' => $item->repostOf])
          @elseif (! blank($item->content) && $item->repostOf)
            {{-- quote repost: show the post content and a nested quoted post --}}
            <p>{!! $item->content !!}</p>
            <ul class="mt-3">
              <li>
                @include('partials.profile.post', ['item' => $item->repostOf])
