<div class="flex items-center gap-1">
  <form action="{{ $hasLiked ? route('posts.unlike', ['profile' => $post->profile, 'post' => $post]) : route('posts.like', ['profile' => $post->profile, 'post' => $post]) }}" method="POST" class="inline">
    @csrf
    <button 
      type="submit" 
      aria-label="{{ $hasLiked ? 'Unlike' : 'Like' }}" 
      @class(['hover:text-pixl', 'text-pixl' => $hasLiked])
      dusk="like-post-button"
    >
      <svg
  ...
  <span @class(['text-sm', 'text-pixl' => $hasLiked]) dusk="like-post-count">{{ $count }}</span>
</div>
