<div class="flex items-center gap-1">
  <form action="{{ $hasReposted ? route('posts.destroy', ['profile' => $post->profile, 'post' => $post]) : route('posts.repost', ['profile' => $post->profile, 'post' => $post]) }}" method="POST" class="inline">
    @csrf
    <button 
      type="submit" 
      aria-label="{{ $hasReposted ? 'Undo repost' : 'Repost' }}" 
      @class(['hover:text-pixl', 'text-pixl' => $hasReposted])
    >
      <svg
  ...
  <span @class(['text-sm', 'text-pixl' => $hasReposted])>{{ $count }}</span>
</div>
