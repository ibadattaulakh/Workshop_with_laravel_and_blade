...
<header>
  <div class="profile-cover">
    <img src="{{ $profile->cover_url }}" alt="{{ $profile->display_name }} cover" class="w-full object-cover h-64">
  </div>
  <div
  ...
  >
    ...
    @auth
      ...
      @else
        <x-follow-button :profile="$profile" />
      @endif
    @endauth
  </div>
...
<div class="mt-6 h-full">
  <nav class="overflow-x-auto [scrollbar-width:none]">
    <ul class="flex min-w-max justify-end gap-8 text-sm">
      <li>
        <a 
          href="{{ route('profiles.show', $profile->handle) }}"
