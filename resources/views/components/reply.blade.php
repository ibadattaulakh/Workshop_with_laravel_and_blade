<li class="group/li relative flex items-start gap-4 pt-4">
  ...
  ></div>
  <a href="{{ route('profiles.show', $post->profile->handle) }}" class="isolate shrink-0">
    <img
  ...
  <div class="border-pixl-light/10 grow border-b pt-1.5 pb-5">
    <div class="flex items-center justify-between gap-4">
      <div class="flex items-center gap-2.5">
        <p>
          <a class="hover:underline" href="{{ route('profiles.show', $post->profile->handle) }}">{{ $post->profile->display_name }}</a>
        </p>
        <p class="text-pixl-light/40 text-xs">
          <a href="{{ route('posts.show', ['profile' => $post->profile->handle, 'post' => $post]) }}" class="hover:underline">
